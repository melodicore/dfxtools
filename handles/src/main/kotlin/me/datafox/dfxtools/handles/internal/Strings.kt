...
internal object Strings {
    const val SET_SPACE_INFER =
        "This constructor may only be used with at least one handle to infer the space, " +
            "use the other constructor that takes in an explicit space argument."
    const val MAP_SPACE_INFER =
        "This constructor may only be used with at least one entry to infer the space, " +
            "use the other constructor that takes in an explicit space argument."
    ...
    const val SPACE_SUBHANDLE_CREATION =
        "Creating subhandles for space handles manually is not permitted " +
    ...
    fun invalidQualifiedHandleId(id: String) =
        "Id \"$id\" is invalid, a fully qualified id " +
            "may contain a single colon (:) and must contain an at symbol (@) after the optional colon."

    fun qualifiedHandleNoSpace(id: String) =
        "\"$id\" is not a fully qualified handle id, " +
    ...
    private fun handleNotInSpace(type: String, space: Space, handle: Handle) =
        "This handle $type " +
