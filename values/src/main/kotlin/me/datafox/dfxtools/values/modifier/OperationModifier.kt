...
class OperationModifier(
    ...
    init {
        if(operation.parameterCount > params.size) {
            logThrow(logger, "${params.size} parameters given to $this but $operation needs ${operation.parameterCount}") { IllegalArgumentException(it) }
        }
        if(operation.parameterCount < params.size) {
            logger.warn { "${params.size} parameters given to $this but $operation only needs ${operation.parameterCount}, some parameters will be ignored" }
        }
    }
